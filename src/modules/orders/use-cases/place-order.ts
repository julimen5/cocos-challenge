...
export async function placeOrder(orderData: PlaceOrderData) {
    ...
    let marketData;
    try {
        marketData = await getCurrentMarketDataOrFail(orderData.instrumentId, 'ACCIONES');
    } catch (error) {
        log.error('No market data found for instrument');
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (orderData.type === 'MARKET') {
            ...
            if (!cashInstrument) {
                ...
                throw new OrderError('Cash instrument not found', 'CASH_INSTRUMENT_NOT_FOUND');
            }
